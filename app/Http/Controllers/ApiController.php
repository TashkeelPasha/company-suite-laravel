...
class ApiController extends BaseController
{
    ...
    private function teamPassengerList(array $types): array { $m = $this->teamMember(); return Passenger::where('company_id', $m->company_id)->with('updates')->latest('id')->get()->map(fn ($p) => $this->passengerJson($p, true, $types))->all(); }
    ...
    private function passengerJson($p, bool $latest = false, array $types = []): array { $r = ['id' => $p->id, 'companyId' => $p->company_id, 'name' => $p->name, 'seatNumber' => $p->seat_number, 'cnic' => $p->cnic, 'flightNumber' => $p->flight_number, 'fromLocation' => $p->from_location, 'toLocation' => $p->to_location, 'departureTime' => $this->dateValue($p->departure_time), 'arrivalTime' => $this->dateValue($p->arrival_time), 'createdAt' => $this->dateValue($p->created_at)]; if ($latest) { $updates = $p->relationLoaded('updates') ? $p->updates : $p->updates()->with('teamMember')->get(); $r['latestStatus'] = $updates->sortByDesc('id')->first()?->status; $r['latestStatusAt'] = $this->dateValue($updates->sortByDesc('id')->first()?->created_at); } return $r; }
